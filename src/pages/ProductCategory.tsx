...
export const ProductCategory: React.FC = () => {
  ...
  return (
    <div className="pt-24">
      ...
      {/* Brands Display */}
      {categoryId === 'audio' ? (
        <section className="bg-bg-main flex flex-col gap-32 py-24">
          {section.brands.map((brand, index) => (
             <div key={brand.name} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-b border-white/5 pb-32 last:border-0 last:pb-0">
               {/* Main Gear Section */}
               <motion.div 
                 ...
                 className="flex flex-col lg:flex-row gap-16 lg:items-center mb-24"
               >
                 ...
                 {brand.image && (
                   <div className="flex-1">
                     <img src={brand.image} alt={brand.name} className="w-full rounded-2xl border border-white/5 object-cover" />
                   </div>
                 )}
               </motion.div>

               {/* Subcategories (Other things they sell) */}
               {brand.subcategories && brand.subcategories.length > 0 && (
                 <motion.div
                   ...
                   transition={{ delay: 0.2 }}
                 >
                   <h3 className="text-2xl font-black mb-12 text-center text-primary uppercase">{brand.name} Products</h3>
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                     {brand.subcategories.map(sub => (
                       <Card key={sub.name} className="flex flex-col h-full bg-bg-panel/20 p-8 border-white/5 hover:border-white/20">
                         ...
                         {sub.image && (
                           <img src={sub.image} alt={sub.name} className="mt-4 rounded-lg w-full h-32 object-cover" />
                         )}
                       </Card>
                     ))}
                   </div>
